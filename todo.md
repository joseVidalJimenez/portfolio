...
- [x] **B-5** `sitemap.html` and `thanktyoupage.html` have `<title>About</title>` — change to correct titles
- [x] **B-6** `contact.html` `<title>contact</title>` — change to `Contact – José Vidal`
...
- [x] **B-9** `curler.html` and `planter.html` — `<script>` appears after `</body>` — move inside `<body>`
- [ ] **B-11** `portfolio.html` — two cards link to the same `curler.html` with no fragment anchors — add `#section` IDs
- [x] **B-12** `contact.html` — submit button hover turns green (legacy rule in `styles.css`) — scope or remove the rule
...
- [ ] **A-1** No `aria-current="page"` on active nav link across all 14 pages
...
- [x] **A-4** `contact.html` `<textarea>` missing `required` attribute — can be submitted empty
...
- [ ] **M-1** Navbar copy-pasted across all 14 HTML files — consider a JS fetch partial or static-site generator
- [ ] **M-2** Duplicate CSS blocks in `styles.css` (`.about-section`, `.listing-section`) — delete the duplicate definitions
- [ ] **M-3** Accent colour `#cb4444` hardcoded 30+ times — define `--accent` CSS variable in `:root`
- [ ] **M-4** Page-specific `<style>` blocks in `curler.html`, `hapicFinger.html`, `RCcar.html` — move to `project.css`
...
- [x] **SC-1** YouTube embeds use `youtube.com` — switch to `youtube-nocookie.com` in all `.yt-facade` JS handlers
